...
class EveningSurveyParser(SurveyParserInterface):
    ...
    def process_raw_data(self, raw_survey_array):
        ...
        for raw_survey_json in raw_survey_array:
            ...
            decrypted_survey_data_json = subprocess.check_output(["node", "decrypt.js", encrypted_survey_data_json])
    ...
    def insert_data_into_harvard_survey(self, survey_data, mysql_config_file):
        """
        ...
        # connect to db
        db = self.connect_to_database(mysql_config_file, "HarvardDev")
        cursor = db.cursor()
        ...
        data = (userID, completionTime, json.dumps(survey_data), whenInsertedReadableTs, responseUUID)
...
if __name__ == '__main__':
    ...
    # initialize parser
    evening_survey_parser = EveningSurveyParser()

    #fetch evening survey data
    raw_survey_data_list = evening_survey_parser.fetch_raw_data({"bucket_name": 'sara-dev-data-storage', "directory": surveyDirectory})

    # process evening survey data
    processed_survey_data_list = evening_survey_parser.process_raw_data(raw_survey_data_list)

    # process evening survey data
    evening_survey_parser.store_processed_data(processed_survey_data_list)

    evening_survey_parser.post_parsing_cleanup({"bucket_name": bucketName, "source_s3_directory": surveyDirectory, "destination_s3_directory": processedDirectory})
