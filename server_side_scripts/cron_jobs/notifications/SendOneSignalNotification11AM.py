...
def send_reminder_notification(player_id, user_id):
    ...
    heading = "Input last night's sleep"
    quote_text = "Sleep survey is now available until 2PM"

    p1 = SendOneSignalNotification(quote_text, heading, player_id, 'sleep', 'sleep_icon.png')
    p1.addUserID(user_id)
    p1.sendOneSignalNotificationsWithTZWithID('11:00AM')
...
if current_time.hour==0 and current_time.minute <30 and current_time.minute >=15:
    ...
    with open("/home/ec2-user/SARA-H/notifications/cron_reminder.txt", "a") as myfile:
        myfile.write("It is not time to send 11AM reminder notifications: " + current_time.strftime("%H:%M:%S") + "\n")
