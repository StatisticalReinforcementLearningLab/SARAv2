...
function fromBackendResponse(authWrapper, resource) {
    ...
    if (resource[PREFIXES_KEY]) {
        for (const path of resource[PREFIXES_KEY]) {
            ...
            const reference = authWrapper.makeStorageReference(new Location(authWrapper.bucket(), pathWithoutTrailingSlash));
            listResult.prefixes.push(reference);
        }
    }
    if (resource[ITEMS_KEY]) {
        for (const item of resource[ITEMS_KEY]) {
            const reference = authWrapper.makeStorageReference(new Location(authWrapper.bucket(), item['name']));
...
class UploadTask {
     ...
     */
    on(type, nextOrObserver = undefined, error = undefined, completed = undefined) {
