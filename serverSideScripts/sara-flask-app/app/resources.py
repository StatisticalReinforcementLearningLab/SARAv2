...
class UserRegistration(Resource):
    def post(self):
        ...
        new_user = UserModel(
            ...
            userinfo_for_admin = json.dumps({"isActive":False, "isParent":False,"onDates":[],"numberTimesOn":0,"numberOnDays":0})
        )
        try:
            new_user.save_to_db()
...
class UserLogin(Resource):
    def post(self):
        ...
        if UserModel.verify_hash(data['password'], current_user.password):
            ...
            access_token = create_access_token(identity = data['username'], expires_delta=expires24hrs)
            #access_token = create_access_token(identity = data['username'])
            #refresh_token = create_refresh_token(identity = data['username'])
            refresh_token = create_refresh_token(identity = data['username'], expires_delta=expires365days)
            ...
            return {
                ...
                } , 201, {'Access-Control-Allow-Origin': '*','Access-Control-Allow-Headers': 'Content-Type','Access-Control-Allow-Methods': 'POST, GET, OPTIONS, DELETE' }
...
class UserInfoForAdmin(Resource):
    ...
    def get(self):
        ...
        #  return {'userName': ''}, 201, {'Access-Control-Allow-Origin': '*'}
        #else: 
        return json.loads(user.userinfo_for_admin), 201, {'Access-Control-Allow-Origin': '*'} 
...
class GetInspirationalQuote(Resource):
    def post(self):
      ...
      cursor = db.cursor()
      cursor.execute("SELECT author_image, author_name, quote_text, date FROM 4PMNotifications where user_id='" + user_id +"' and is_sent = 1 order by whenSentTs DESC")
...
class GetUnlockedIncentives(Resource):
    def post(self):
      ...
      cursor.execute("SELECT user_id, incentiveString FROM UnlockedIncentive where user_id='" + user_id +"' and incentiveType = '" + incentive_type +"' order by whenInserted DESC;")
