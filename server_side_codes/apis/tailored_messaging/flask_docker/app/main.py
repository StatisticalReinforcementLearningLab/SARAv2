...
def pick_a_message(state_list):
    ...
    # only buckets of true states are selecteted.
    ...
    n_samples = 1
    all_buckets = [state.message_buckets[0] for state in state_list]
    # print(all_buckets)
    # sample a number between 0 to lenth of all buckets or 
    ...
    sampled_message = random.sample(all_buckets[selected_bucket[0]]["messages"], 1)[0]
    return (
        sampled_message,
...
def get_message():
    ...
    message_bucket.add_csv_to_bucket(BUCKET_DIRECTORY + "1_bucket_weekend.csv")
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(message_bucket.return_all_messages(), 1, "weekend==True")
    ...
    # Add message buckets
    message_bucket = MessageBucket()
    message_bucket.add_csv_to_bucket(BUCKET_DIRECTORY + "2_bucket_neg_mood.csv")
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(message_bucket.return_all_messages(), 1, "neg_mood>0")
    if state_condition == True:
        state_list.append(state)
    ...
    # Add message buckets
    message_bucket = MessageBucket()
    message_bucket.add_csv_to_bucket(BUCKET_DIRECTORY + "3_bucket_adherence_motivation.csv")
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(
        message_bucket.return_all_messages(), 1, "adherence_motivation<4"
    )
    if state_condition == True:
        state_list.append(state)
    else:
    ...
    # Add message buckets
    message_bucket = MessageBucket()
    message_bucket.add_csv_to_bucket(BUCKET_DIRECTORY + "4_bucket_location.csv")
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(message_bucket.return_all_messages(), 1, "location_home_false")
    if state_condition == True:
        state_list.append(state)
    else:
    ...
    message_bucket.add_csv_to_bucket(
        BUCKET_DIRECTORY + "5_social_company_alone.csv"
    )  # change
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(
        message_bucket.return_all_messages(), 1, "social_alone_true"
    )  # change
    if state_condition == True:
        state_list.append(state)
    else:
    ...
    message_bucket = MessageBucket()
    message_bucket.add_csv_to_bucket(
        BUCKET_DIRECTORY + "6_social_company_friends.csv"
    )  # change
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(
        message_bucket.return_all_messages(), 1, "social_friends_true"
    )  # change
    if state_condition == True:
        state_list.append(state)
    else:
    ...
    # Add message buckets
    message_bucket = MessageBucket()
    message_bucket.add_csv_to_bucket(BUCKET_DIRECTORY + "7_fatigue.csv")  # change
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(
        message_bucket.return_all_messages(), 1, "fatigue>0"
    )  # change
    if state_condition == True:
        state_list.append(state)
    else:
    ...
    # Add message buckets
    message_bucket = MessageBucket()
    message_bucket.add_csv_to_bucket(BUCKET_DIRECTORY + "8_nausea.csv")  # change
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(
        message_bucket.return_all_messages(), 1, "nausea>0"
    )  # change
    if state_condition == True:
        state_list.append(state)
    else:
    ...
    message_bucket = MessageBucket()
    message_bucket.add_csv_to_bucket(
        BUCKET_DIRECTORY + "9_parent_disagreement.csv"
    )  # change
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(
        message_bucket.return_all_messages(), 1, "cg_aya_disagreement_true"
    )  # change
    if state_condition == True:
        state_list.append(state)
    else:
    ...
    # Add message buckets
    message_bucket = MessageBucket()
    message_bucket.add_csv_to_bucket(BUCKET_DIRECTORY + "10_postive_mood.csv")
    message_bucket.retrieve_messages_from_csv()
    state.add_message_bucket(message_bucket.return_all_messages(), 1, "pos_mood>2")
    if state_condition == True:
        state_list.append(state)
    else:
    ...
    return_object_json = {}
    return_object_json["sampled_message"] = sampled_message
    return_object_json["sampled_bucket"] = sampled_bucket
    return_object_json["all_relevant_buckets"] = all_buckets
    return_object_json["message_bucket_prob_list"] = message_bucket_prob_list
    return_object_json["survey_processed"] = message_tailor_object


    # return json.dumps(return_object_json, indent=4)
    response = jsonify(return_object_json)
